...
2. To install GoSublime, SidebarEnhancements and Go Build, press `Ctrl+Shift+p` open Package Control, then type `pcip` (short for "Package Control: Install Package"), restart again.
	...
	Now you type GoSublime, press OK to install, same steps for installing SidebarEnhancements and Go Build.
3. To verify if installation is succeed, open Sublime, then open file `main.go` to see if it has syntax highlight, type `import` to see if it has prompts, after type `import "fmt"`, type `fmt.` to see if it has intelligent completion for functions.
...
6. Congratulations! speedbar is closed as deafult, cut comment sign in line `;;(speedbar 1)` to have this feature, or you can have it through `M-x speedbar`.

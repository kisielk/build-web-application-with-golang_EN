...
In ths section, I'm going to show you some IDEs that have abilities of intelligent completion and auto-format. There are all cross-platform, so the steps I show you should not be very different.

##LiteIDE
LiteIDE is an open source lightweight IDE for developing Go project only, made by visualfc.
...
- Compile environment
	Switch configuration in LiteIDE denpends on your operating system.
	...
	Replace `GOROOT=c:\go` to your Go installation path, save it. If you have MinGW64, you can add `c:\MinGW64\bin` to your path environment variable for `cgo` support.
	
	In Linux and 64-bit version of Go, you should choose linux64 in evironment configuration in tool bar. Then you choose `opinion`, find `LiteEnv` in the left list, open file `linux64.env` in the right list.
...
- $GOPATH
	$GOPATH is the path that contains project list, open command tool (press `Ctrl+` in LiteIDE) then type `go help gopath` for more details.
	It's very easy to view and change $GOPATH in the LiteIDE. `View - Setup GOPATH` to view and change these values.
...
First, download [Sublime](http://www.sublimetext.com/) depends on your operating system.

1. Press Ctrl+` open command tool, input following command.

		import urllib2,os; pf='Package Control.sublime-package'; ipp=sublime.installed_packages_path(); os.makedirs(ipp) if not os.path.exists(ipp) else None; urllib2.install_opener(urllib2.build_opener(urllib2.ProxyHandler())); open(os.path.join(ipp,pf),'wb').write(urllib2.urlopen('http://sublime.wbond.net/'+pf.replace(' ','%20')).read()); print 'Please restart Sublime Text to finish installation'
		
	Restart when installatin finished. Then you can find `Package Control` item in the Preferences.
	
	![](images/1.4.sublime3.png?raw=true)
	
	Figure 1.7 Sublime Package Control
2. To install GoSublime, SidebarEnhancements and Go Build, press `Ctrl+Shift+p` open Package Control, then type `pcip` (short for "Package Control: Install Package"), restart again.

	![](images/1.4.sublime4.png?raw=true)
	
	Figure 1.8 Sublime Install Packages
	
	Now you type GoSublime, press OK to install, same steps for SidebarEnhancements and Go Build.
3. To verify if installation is succeed, open Sublime, open file `main.go` to see if it has syntax highlight, type `import` to see if it has prompts, after type `import "fmt"`, type `fmt.` to see if it has intelligent completion for functions.

	If everything is fine, you're all set.
	
	If not, check your $PATH again. Open terminal, type `gocode`, if it cannot run, your $PATH must not config correctly.
...
Figure 1.8 Vim ntelligent completion for Go

1. Syntax highlight for Go

		cp -r $GOROOT/misc/vim/* ~/.vim/
		
2. Set syntax hightlight on
...
3. Install [gocode](https://github.com/nsf/gocode/)
	...
	gocode will be installde in `$GOBIN` as default
4. Config [gocode](https://github.com/nsf/gocode/)
...
##Emacs
Emacs is so-called Weapon of God. She it not only a editor, also a powerful IDE.
...
2. Install [gocode](https://github.com/nsf/gocode/)
	...
	gocode will be installde in `$GOBIN` as default
3. Config [gocode](https://github.com/nsf/gocode/)
...
6. Congratulations! speedbar is closed as deafult, cut comment sign in line `;;(speedbar 1)` to open this feature, or you can open through `M-x speedbar`.
...
5. Config plugins.
	...
	(1).Config Go compiler
	
	![](images/1.4.eclipse2.png?raw=true)
	
	Figure 1.12 Go Setting in Eclipse
	
	(2).Config gocode(optional), set gocode path to where the gocode.exe is.
	
	![](images/1.4.eclipse3.png?raw=true)
	
	Figure 1.13 gocode Setting
	
	(3). Config gdb(optional), set gdb path to where the gdb.exe is.
...
6. Check installation
	...
	Test installation as show following.(you need to type command in console in Eclipse)
	
	![](images/1.4.eclipse6.png?raw=true)
	
	Figure 1.16 Test Go progem in Eclipse
...
1. Downlaod IDEA, there is no different from Ultimate and Community Edition
